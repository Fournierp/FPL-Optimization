...
def write() -> None:
    ...
    with st.expander('Parameters', expanded=True):
        ...
        with col1:
            gk_weight = st.slider('GK Weight', min_value=0.01, max_value=1.0, value=0.03, step=0.02)
        with col2:
            first_bench_weight = st.slider('1st Weight', min_value=0.01, max_value=1.0, value=0.21, step=0.02)
        with col3:
            second_bench_weight = st.slider('2nd Weight', min_value=0.01, max_value=1.0, value=0.06, step=0.02)
        with col4:
            third_bench_weight = st.slider('3rd Weight', min_value=0.01, max_value=1.0, value=0.01, step=0.02)

        col1, col2, col3, col4, col5 = st.columns(5)
        with col1:
            decay = st.slider('Decay rate', min_value=0.0, max_value=1.0, value=0.9, step=0.02)
        with col2:
            vicecap_decay = st.slider('Vicecap rate', min_value=0.0, max_value=1.0, value=0.1, step=0.02)
        with col3:
            ft_val = st.slider('FT value', min_value=0.0, max_value=5.0, value=1.5, step=0.2)
        with col4:
            hit_val = st.slider('Hit value', min_value=2.0, max_value=8.0, value=6.0, step=0.5)
        with col5:
            itb_val = st.slider('ITB value', min_value=0.0, max_value=1.0, value=0.008, step=0.02)
    ...
    if st.button('Run Optimization'):
        ...
        with st.spinner('Running Optimization ...'):
            to = Team_Optimization(
                team_id=team_id, horizon=horizon, noise=False, premium=True if premium == 'Premium' else False
            )
            ...
            for j, row in to.initial_team_df.iterrows():
                rectangle = patches.Rectangle((0, 14 - j), 12, 0.75, facecolor=color_position[row['Pos']])
            ...
            for i, gw in enumerate(np.sort(df.GW.unique())):
                ...
                for j, row in df_gw.iterrows():
                    rectangle = patches.Rectangle(
                        ((i + 1) * 16, 14 - j), 12, 0.75, facecolor=color_position[row['Pos']]
                    )
                    ax.add_patch(rectangle)
                    rx, ry = rectangle.get_xy()
                    cx = rx + rectangle.get_width() / 2.0
